...
export default function SignageManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      let playersQuery = supabase.from('media_players').select('*, hardware_devices(*), stores!inner(id, name, company_id, companies!inner(id, name, concept_id))').eq('player_type', 'signage').order('name');
      ...
      const [playersRes, devicesRes, storesRes, displaysRes] = await Promise.all([
        playersQuery,
        supabase.from('hardware_devices').select('*').in('status', ['available']).eq('usage_type', 'signage').order('device_id'),
        storesQuery,
