  ...
  const loadAccessibleStores = async () => {
    ...
    try {
      if (!userId) {
        ...
        // Check if user has store-level access
        ...
        if (accessData && accessData.length > 0) {
          ...
          // No store-level access, fall back to company/concept access
          const { data: profile } = await supabase
            .from('user_profiles')
            .select('company_id, concept_id, store_id')
            .eq('id', userId)
            .maybeSingle();

          if (profile?.store_id) {
            // User has single store access
            const { data: stores, error } = await supabase
              .from('stores')
              .select(`
                id,
          ...
          } else if (profile?.concept_id) {
            // User has concept-level access
